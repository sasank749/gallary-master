...
class _UploadStatusState extends State<UploadCommunity> {
  ...
  Future uploadStatus() async {
    ...
    String docId=Firestore.instance.collection("Community").document().documentID;
    statusModel.docid=docId;
    await Firestore.instance.collection("Community").document(statusModel.docid).setData(statusModel.toMap());
    Fluttertoast.showToast(msg: "status uploaded");
    Navigator.pop(context);
    return imageUpload;
  }
  Future<dynamic> uploadImage() async{
    StorageReference storageReference=await FirebaseStorage().ref().child("Community");
