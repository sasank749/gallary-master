...
class UploadHousehold extends StatefulWidget {
  @override
  _UploadStatusState createState() => _UploadStatusState();
}

class _UploadStatusState extends State<UploadHousehold> {
  ...
  Future uploadStatus() async {
    ...
    String docId=Firestore.instance.collection("Household").document().documentID;
    statusModel.docid=docId;
    await Firestore.instance.collection("Household").document(statusModel.docid).setData(statusModel.toMap());
    Fluttertoast.showToast(msg: "status uploaded");
    Navigator.pop(context);
    return imageUpload;
  }
  Future<dynamic> uploadImage() async{
    StorageReference storageReference=await FirebaseStorage().ref().child("Household");
